...
def preprocess_imagemat(vid, shape_r, shape_c):
    # Assumes images are rgb from 0 to 1
    # N x H x W x 3
    vid = (vid*255).astype(np.uint8)
    ims = np.zeros((len(vid),shape_r,shape_c,3))
    for i in range(len(ims)):
        img = cv2.cvtColor(vid[i],cv2.COLOR_RGB2BGR)
